...
// Save to `path`, 
pub fn save_to<P: AsRef<Path> + std::fmt::Display>(image: &DynamicImage, compression: u8, path: P) {
    let mut buf = File::create(&path).expect( format!("Failed to create `std::fs::File` on `{}`", path).as_str() );
